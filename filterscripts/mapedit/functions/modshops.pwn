GetModShopName(modshop) {
	...
	switch(modshop) {
		case MODSHOP_TRANSFENDER: {
			strunpack(name, !"Transfender");
		}
		case MODSHOP_LOCOLOW: {
			strunpack(name, !"Loco Low Co");
		}
		case MODSHOP_WHEELARCH: {
			strunpack(name, !"Wheel Arch Angels");
		}
		default: {
			strunpack(name, !"Unknown Tuning Shop");
		}
	}
	return name;
}

GetModShopPosition(modshop, &Float:x, &Float:y, &Float:z, &Float:a) {
	switch(modshop) {
		case MODSHOP_TRANSFENDER: {
